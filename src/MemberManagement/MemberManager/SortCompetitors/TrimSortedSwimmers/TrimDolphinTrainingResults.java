package MemberManagement.MemberManager.SortCompetitors.TrimSortedSwimmers;

import MemberManagement.MemberManager.SortCompetitors.CreateTrainingResults.TrainingResult;
...
public class TrimDolphinTrainingResults implements TrimTrainingResults{
    ...
    public List<TrainingResult> trim(List<TrainingResult> results) {
        ...
        if(!optionalDiscipline.isPresent())
            return null;
        final MemberManagement.Discipline[] tempDis = {optionalDiscipline.get()};
        final int[] index = {0};
